...
import pandas as pd, subprocess, csv, sys, time, ast, socket
...
class ProcessTraffic:
    ...
    def process_csv(self, input_csv: str, pcap_path: str) -> Dict:
        ...
        try:
            dataframe = pd.read_csv(str(input_csv_path))
        except Exception as e:
            # If the existing CSV is corrupt (e.g. "EOF inside string"), remove it,
            # regenerate from the pcap and retry once to avoid persistent parse errors.
            print(f"CSV parse error ({input_csv_path}): {e} — regenerating CSV and retrying")
            try:
            ...
            regenerated = self.generate_csv(self.input_pcap)
            try:
                dataframe = pd.read_csv(str(regenerated))
                input_csv_path = Path(regenerated)
            except Exception as e2:
                print(f"Failed to parse regenerated CSV ({regenerated}): {e2}")
                raise
...
if __name__ == '__main__':
    logger = CustomLogger("PCAP Processing (TrafficParameters)", logfile_name="pcap_processing")
    load_dotenv()
    
    while True:
        row = db_helper.get_unchecked_traffic_parameters()
        
        if row:
            logger.info(f"Retrieved row: {row}")
            # Extract owner and name from the repository name (format: "owner/name")
            repo_owner, repo_name = row.name.split("/")
            hostname = row.processing_host
            # PCAP naming convention: {repo_owner}_{repo_name}_{hostname}_run_0.pcap
            pcap_path = f"pcap/{repo_owner}_{repo_name}_{hostname}_run_0.pcap"
            ...
            try:
                ...
                success, message = db_helper.update_traffic_parameters(row.id, **payload)
                if success:
                    logger.info(message)
                else:
                    logger.warning(message)
            except FileNotFoundError as e:
                logger.error(f"PCAP file not found at {pcap_path}: {e}")
                try:
                    success, message = db_helper.update_traffic_parameters(row.id, application_flow={"skip": True})
                    if success:
                        logger.info(f"Marked row {row.id} as skipped (missing pcap)")
                    else:
                        logger.warning(f"Failed to mark skip for row {row.id}: {message}")
                except Exception as db_e:
                    logger.error(f"Failed to update DB for skip on row {row.id}: {db_e}")
            except Exception as e:
                logger.error(f"Error processing PCAP: {e}")
                try:
                    success, message = db_helper.update_traffic_parameters(row.id, application_flow={"skip": True})
                    if success:
                        logger.info(f"Marked row {row.id} as skipped (processing error)")
                    else:
                        logger.warning(f"Failed to mark skip for row {row.id}: {message}")
                except Exception as db_e:
                    logger.error(f"Failed to update DB for skip on row {row.id}: {db_e}")
